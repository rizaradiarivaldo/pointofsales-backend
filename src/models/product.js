...
const product = {
  getAll: (name, sort, typesort, limit, offset) => {
    return new Promise((resolve, reject) => {
      const query = `SELECT *, (SELECT COUNT(*) FROM products) as count FROM products LEFT JOIN category on products.id_category=category.id_category WHERE productname LIKE '%${name}%' ORDER BY ${sort} ${typesort} LIMIT ${limit} OFFSET ${offset} `;
          ...
          if (err) {
            ...
            resolve(
              new Promise((resolve, reject) => {
                ...
                db.query(
                  `UPDATE products SET productname='${data.productname}',image='${imagename}',price='${data.price}',id_category='${data.id_category}' WHERE id_product='${id}'`,
                  (err, resultUpdate) => {
                    if (err) {
                      reject(new Error(err));
                    } else {
